...
`:"",iconTextField=(e,t)=>`<div class="icon ${e}"></div><span>${t}</span>`,icon=e=>'<div class="icon '+e+'"></div>';function infoRepo(t){const e=repos.find(e=>e.name===t);var o="";return o+=`<h1><a href="${"https://github.com/"+gitUser.login+"/"+e.name}" target="${e.name}">
    ...
    `};let topicList=[];const maxTopics=config.maxTopics||3,htmlTopic=e=>`<div><a href="https://github.com/topics/${e}" target="t_${e}">${e}</a></div>`,htmlTopics=e=>'<div id="my_topics" class="field topics">'+(topicList=e).slice(0,maxTopics).map(htmlTopic).join("")+(e.length>maxTopics?'<div class="morelink" onclick="moreTopics(event)">+ '+(e.length-maxTopics)+" more...</div>":"");function moreTopics(e){e.stopPropagation(),document.getElementById("my_topics").innerHTML=topicList.map(htmlTopic).join("")}const repoItemPop=e=>{var t=`https://github.com/${gitUser.login}/${e.name}/`,o=` target="${e.name}" rel="noopener"`;return`

...
<span id="summary-count">${repos?.length||""}</span></div>`,sortOptions=()=>`<div><span class="sort-dir" onclick="javascript:onSort(event,'n')">N</span>/<span class="sort-dir" onclick="javascript:onSort(event,'s')">S</span></div>`,searchSortElems=()=>`<div class="field-holder">
  ...
  ${sortOptions()}
    ...
    <div class="icon link"></div> <a href="${e[t]}" target="${e.name}" rel="noopener">${e[t]}</a>
      ...
      ${e.name}
    </a></h1>`,o+=urlField(e,"homepageUrl")+repoItemPop(e)+(e.description?`<div class="field f-desc">${e.description}</div>`:"")+textField("Updated",e.updatedAt)+textField("Created",e.createdAt),o+=e.licenseInfo?`<div class="field multi">${icon("license")}<a href="${e.licenseInfo.url}" target="${e.licenseInfo.key}" rel="noopener">${e.licenseInfo.name}</a></div>`:"",o+=e.languages&&e.languages.length?e.languages.map(e=>`<span><div style="background-color:${e.color}" class="ln-dot"></div>${e.name}</span>`).join(""):"",o+=htmlTopics(e.topics),o+="</div>"},avatarPix=(e,t)=>`<div class="${t}"><img src="${e}"></div>`,userTooltip=e=>avatarPix(fans[e.id].avatarUrl,"avatar-small")+e.id+textField("Updated",e.updatedAt),infoUser=e=>{var t="*"===e?gitUser:fans[e];let r="";var o=t.login===gitUser.login,e="https://github.com/"+t.login;return r+=t.avatarUrl?'<div class="h190"><img src="'+t.avatarUrl+'"></div>':"",r+=`<div class="profile-top"><h1><a href="${e}" target="${t.login}">${t.login}</a></h1>`,r+=t.fullName?`<div class="fullName">${t.fullName}</div>`:"",r+=urlField(t,"websiteUrl")+urlField(t,"webURL"),r+=t.location?`<div class="field"><div class="icon location"></div> <span>${t.location}</span></div>`:"",r+=t.bio?`<div class="field">${t.bio}</div>`:"",r+='<div class="field multi">',r+=t.nbFollowers?`${icon("followers")}<a href="https://github.com/${t.login}?tab=followers" target="${"follow_"+t.name}" rel="noopener">${t.nbFollowers}</a>`:"",r+="</div>",r+=o?condIcon("star",t.nbStars)+condIcon("fork",t.nbForks):t.nbRepos?`${icon("repos")}<a href="https://github.com/${t.login}?tab=repositories" target="${t.name}" rel="noopener">${t.nbRepos}</a>`:"",r+="</div>",o?r+=searchSortElems()+repoList(gitUser.repos,!0,!0):(t.starred&&t.starred.length&&(r+=`<div class="field">
      <label class="lbl-10">Starred: ${t.starred.length}</label>
      ${repoList(repoArr(t.starred),!1,!0,null,!1)}
      </div>`),t.forked&&t.forked.length&&(r+=`<div class="field">
      <label class="lbl-10">Forked: ${t.forked.length}</label>
      ${repoList(repoArr(t.forked),!1,!0,null,!1)}
      </div>`)),r},repoList=(e,t,r,o,s)=>{var i=e.map(e=>repoItem(e,t,s)).join(""),e=r?"":'<span class="repoIco">'+icon("repos")+" "+e.length+"</span>";return o?i:'<div id="reposList" class="reposList">'+e+i+"</div>"},repoItem=(e,t,r=!0)=>{var o=e.login||"*"===e.name;if(o&&t)return"";var s=o?e.login:e.name;if(!s)return"";t=o?e.login:gitUser.login+"/"+e.name;return`
      ...
      <a class="${o?"gituser":""}" href="javascript:selectProject('${e.name}')">
        ...
        <a class="${o?"gituser":""}" href="https://github.com/${t}" target="${s}" rel="noopener">
          ${s}
        </a>
         </div>
        ${o&&r?`<div class="field multi">
...
<div class="field multi">
  ${e.nbStars?`<a href="${t}stargazers" ${r}>${icon("star")}${e.nbStars}</a>`:""}
  ${e.nbForks?`<a href="${t}network/members" ${r}>${icon("fork")}${e.nbForks}</a>`:""}
  ${e.nbStars?`<a href="https://star-history.com/#${gitUser.login}/${e.name}" ${r}>${icon("chart")}</a>`:""}
</div>
`};
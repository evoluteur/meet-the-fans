...
const runQuery = (q, cb, cbError) => {
    ...
    fetch(apiPathGraphQL, gqlOptions(q))
        ...
        .then(data => {
            if (data.errors) {
                setStatus('<span class="red">' + data.errors.map(err => err.message).join('<br/>') + '</span')
                if(cbError) {
                    cbError(data.errors)
                }
                cb(null, true)
...
const cleanUser = (node, repo) => {
    const user = {
        ...
        nbStarredRepos: node.starredRepositories ? node.starredRepositories.totalCount : 0,
...
const cleanRepo = r => ({
    ...
    version: r.repoRelease.nodes && r.repoRelease.nodes.length ? r.repoRelease.nodes[0].name : '',
    //releasedAt: r.releases.nodes[0],
    topics: r.repositoryTopics ? r.repositoryTopics.nodes.map(rt => rt.topic.name || 'N/A') : null
})
  ...
  `
    ...
    function cbRepos(data) {
        ...
        if (isFirstUserQuery) {
            ...
            document.getElementById('user').value = 'const gitUser = ' + JSON.stringify(user, null, 2);
        }
        if (data.user.followers && data.user.followers.pageInfo.hasNextPage) {
            ...
            runQuery(qRepos(data.user.followers.pageInfo.endCursor), cbRepos, cbError);
        } else {
...
function getFans(repo) {
  ...
  `
    ...
    function cbFans(data, hasError) {
        ...
        if (!runningQueries) {
            setStatus('Data fetched in ' + Math.floor((new Date() - startTime) / 1000) + ' seconds.')
...
const dataSummary = () => user.nbRepos + ' repos, ' +
    user.nbStars + ' stars, ' +
    user.nbForks + ' forks, ' +
    user.nbFollowers + ' followers => ' +
    Object.keys(fans).length + ' fans.'
...
function download() {
    ...
    const txt = `
/*
  ${user.login} on ${dNow.toLocaleDateString()} ${dNow.toLocaleTimeString('en-US')}
